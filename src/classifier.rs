...
// ---------------------------------------------------------------------------
// Baked-in default models.
//
// The shipped HOR-score and k-predictor random forests are compiled into
// the binary via `include_bytes!`. This keeps `kitehor` self-contained:
// a single binary with no data files to install or discover. Users who
// trained their own model can still pass `--hor-model <path>` /
// `--k-model <path>` on the CLI — `RandomForest::load_json` reads from
// disk just like before.
//
// Paths are relative to this source file (`src/classifier.rs`), so they
// resolve to `<crate root>/models/*.rftrees.json`.
// ---------------------------------------------------------------------------

/// Pre-trained HOR-score random forest (probability mode, 500 trees, 15
/// features). Generated by `tools/training/export_ranger.R`.
pub const BAKED_HOR_MODEL: &[u8] = include_bytes!("../models/hor_score.rftrees.json");

/// Pre-trained k-predictor random forest (regression, 500 trees, 20
/// features). Generated by `tools/training/export_ranger.R`.
pub const BAKED_K_MODEL: &[u8] = include_bytes!("../models/k_pred.rftrees.json");
...
impl RandomForest {
    pub fn load_json<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        Self::parse_json(&text).with_context(|| format!("parsing RF model {:?}", path))
    }

    /// Load a forest from raw JSON bytes (used by the baked-in default
    /// models — see [`BAKED_HOR_MODEL`] / [`BAKED_K_MODEL`]).
    pub fn load_json_bytes(bytes: &[u8]) -> Result<Self> {
        let text = std::str::from_utf8(bytes).context("RF model bytes are not valid UTF-8")?;
        Self::parse_json(text).context("parsing baked-in RF model bytes")
    }
    ...
    /// silently zero-fill).
    pub fn assemble<F: Fn(&str) -> Option<f64>>(&self, lookup: F) -> Vec<f64> {
        self.feature_names
            .iter()
...
/// path is given.
pub const DEFAULT_CONFIG_TOML: &str = include_str!(concat!("../config/classifier.toml"));
...
impl ClassifierConfig {
    pub fn default_baked() -> Result<Self> {
        toml::from_str(DEFAULT_CONFIG_TOML).context("parsing baked-in default classifier.toml")
    }
    ...
    pub fn load<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        toml::from_str(&text).with_context(|| format!("parsing classifier config {:?}", path))
    }
